...
\title{Semantics and Equational Axiomatisation for Quantum Communication}
...
We can in fact give an equivalent operational model for $\quantum$. Briefly, configurations are of the form $\langle \rho, c \rangle$ where $\rho$ is the program state, $c$ the running program, and transitions $\to_p$ are probabilistic. We can then formulate in terms of $\to_p$ an operational notion of program equivalence $\quequiv$ equivalent with denotational equality. 
...
\begin{figure}
  \[
  \config{\tikzfig{meas_out_init}}{\sigma}{\opout(q, c)} \to_1 \config{\tikzfig{out_post}}{\sigma + \opout}{c} 
  \]
...
We have obtained a sound axiomatisation $\iotq$ for one notion of quantum communication, presented in two equivalent ways: operationally and denotationally. However, unlike the quantum computing case, we do not have completeness -- whether $\sem{c}=\sem{c'} \implies c = c'$ is open. Achieving this is our immediate next step. It would equally be interesting relate this work to quantum process calculi, higher order quantum computing \cite{PaganiSV13,kissinger2019categorical}, and their applications in verifying quantum protocols (e.g.~\cite{lqCCS_10.1145/3632885}).
